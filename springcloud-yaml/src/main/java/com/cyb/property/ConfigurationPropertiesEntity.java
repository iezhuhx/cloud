...
/**
 * ConfigurationProperties 注解语法类 第一步：导入依赖 spring-boot-configuration-processor；
 * 第二步：把ConfigurationProperties注解修饰的类添加到Spring的IOC容器中；
 * 第三步：设置prefix属性，指定需要注入属性的前缀； 第四步：添加数据校验注解，开启数据校验；
 *
...
public class ConfigurationPropertiesEntity {
	...
	// @Email // 解析失败，数据校验成功：BindValidationException: Binding validation errors
	// on itdragon
